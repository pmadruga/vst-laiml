...
| T1 identify, model strategy | the 4 pages with risk register rows (51, 71, 73, 74), one call each | 4 | 12.1 s | 3,440 / 1,129 | |
| T2 describe | one call per risk register risk | 10 | 18.8 s | 7,694 / 1,737 | |
| T4 repair | one retry per record failing grounding | 2 | 4.0 s | 1,389 / 381 | |
...
| L1–L6 load | SQLite write, indexes, FTS, run record | 0 | 0.3 s | 0 | risk.db 150–165 KB |
| **Pipeline total** | | **16** | **36 s** | **12,523 / 3,247** | run dir 400 KB |
| API structured endpoints | `/health`, `/risks` with filters and `q`, `/questions/*`, `/runs/*/record` | 0 | 1–2.5 ms median, under 4 ms p95 | 0 | |
| API `/ask` | one intent call, then SQL | 1 | 0.8–1.7 s warm, 7.4 s cold | ~420 / ~120 | |

Notes. The model server is the whole cost: 35 of the 36 seconds. Identify calls are the longest per call (3.0 s against 1.9 s for describe) because each reads a full page. The cold `/ask` figure includes the server loading the model after it was unloaded; warm calls are around one second. Extract, load and the API's structured endpoints are effectively free at this scale.

Variance between live runs. The 2026-09-14 re-run with the same code, prompts and model made 17 calls instead of 16 (three repairs instead of two), 13,223 tokens in and 3,183 out, 40 s of model time, and produced the same nine records with the same one flagged for review. The repair count is the part that moves: it depends on how many first descriptions fail grounding, which varies with sampling on the server. The regression runs sit in the same range: 16 and 17 calls for the broken parser and the prompt without mitigation; 19 calls and 77 s for the weaker ministral 14B, which is slower on this machine and needs more repairs.
...
| Equivalent | Why this one | In / out per M | One report (16 calls) | One `/ask` |
|---|---|---|---|---|
| `openai/gpt-oss-20b` | the same weights as the local model, hosted | $0.03 / $0.13 | $0.0008 | $0.00003 |
| `mistralai/ministral-14b-2512` | the same weights as the weak-model regression, hosted | $0.20 / $0.20 | $0.0032 | $0.0001 |
| `openai/gpt-5-mini` | a small hosted proprietary model | $0.25 / $2.00 | $0.0096 | $0.0003 |
| `anthropic/claude-sonnet-5` | a frontier model; its batch tier is half | $2.00 / $10.00 | $0.058 | $0.002 |

Per step at the frontier price: identify $0.018, describe $0.033, repair $0.007. Describe is the largest line because it runs once per risk.
...
|---|---|---|---|---|---|---|
| 1 | 16 | 12.5k / 3.2k | 36 s | 36 s | 0.4 MB | 9 risks |
| 10 | 160 | 125k / 32k | 6.1 min | 1.5 min | 4 MB | ~90 risks |
| 20 | 320 | 250k / 65k | 12 min | 3.0 min | 8 MB | ~180 risks |
| 50 | 800 | 626k / 162k | 30 min | 7.6 min | 20 MB | ~450 risks |
| 200 (one quarter) | 3,200 | 2.5M / 0.65M | 2.0 h | 30 min | 80 MB | ~1,800 risks |
...
| 20 | $0.02 | $0.06 | $0.19 | $1.15 |
| 50 | $0.04 | $0.16 | $0.48 | $2.88 |
| 200 (one quarter) | $0.16 | $0.63 | $1.92 | $11.50 |

A thousand `/ask` questions cost $0.03 on hosted gpt-oss-20b and $2.03 on the frontier model.
...
- **Review load.** One of nine records was flagged for review on the baseline and on the live re-run, about one per report. At that rate 50 reports produce about 50 flagged records and a quarter about 200, which is analyst time, not machine time. The weaker model flags more.
