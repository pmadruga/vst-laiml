...
def grounding_problems(rec: RiskRecord, page_text: str) -> list[str]:
    ...
    if rec.mitigation:
        src = _norm(rec.mitigation)
        words = _content_words(rec.mitigation)
        if words and len(words & set(page_norm.split())) / len(words) < 0.5:
...
def validate_and_repair(records: list[RiskRecord], parsed: ParseResult, record: RunRecord, client: LLMClient | None) -> list[RiskRecord]:
    ...
    for rec in records:
        ...
        problems = grounding_problems(rec, page_text)
        if problems and client is not None:
            retry_user = (f"Your previous record for the risk '{rec.title}' failed these checks: {', '.join(problems)}. "
                          f"Rewrite it so that every sentence, number and name is supported by the text below, and write exactly 2 to 3 sentences.\n\n"
                          f"Title as printed: {rec.citations[0].span[:0]}{rec.title}\nDescription as printed:\n{rec.verbatim_span}\n"
                          + (f"\nPotential impact as printed:\n{rec.potential_impact}\n" if rec.potential_impact else "")
                          + (f"\nHow we manage it, as printed:\n{rec.mitigation}\n" if rec.mitigation else "\nThe text states no mitigation.\n"))
            try:
                ...
                fixed = None
            candidate = rec if fixed is None else rec.model_copy(update={"title": fixed.title, "description": fixed.description, "category": fixed.category,
                                               "secondary_categories": fixed.secondary_categories, "mitigation": fixed.mitigation,
                                               "confidence": fixed.confidence})
            new_problems = grounding_problems(candidate, page_text) if fixed is not None else problems
            if fixed is not None and len(new_problems) < len(problems):
...
def validate_transform(records: list[RiskRecord], candidates: int, merges: int, parsed: ParseResult, record: RunRecord,
                       replay_records: list[RiskRecord] | None = None) -> str:
    """T6: set-level checks. Evaluators run separately (eval/run_eval.py) and write eval_scores."""
    problems = []
    if len(records) != candidates - merges:
        problems.append(f"count:{len(records)}!={candidates}-{merges}")
    ...
    if replay_records is not None:
        ...
        record.add("transform", "replay_identical", "ok" if same else "error", count=0 if same else 1)
